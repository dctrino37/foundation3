...
{{--             <div class="text-end pb-1">
        ...
        <div class="fright">
            ...
            {{trans('middle_east_office.you_must_be')}} <a href="{{url('/login')}}">{{trans('middle_east_office.logged_in')}}</a> {{trans('middle_east_office.or')}} <a href="{{url('/register')}}">{{trans('middle_east_office.register')}}</a> {{trans('middle_east_office.to_participate_in_this_conversation.')}}
        </div>
        @endif
...
<div class="modal fade" tabindex="-1" role="dialog" data-modal="report-user" data-close-modal>
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content shadow-sm">
            <div class="modal-header">
                <h5 class="modal-title">{{trans('middle_east_office.report_spam')}}</h5>
                <button type="button" class="btn-close" aria-label="Close" data-close-modal></button>
            </div>
                <form action="{{ route('report_user', $thread) }}" method="POST" class="report_user_form">
                {{csrf_field()}}
                @if (isset($method))
                    @method($method)
                @endif
                ...
                {{ trans('forum::general.generic_confirm') }}
                <input type="hidden" name="report_user_id" value="" class="hidden_report_user_id">
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-close-modal>{{ trans('forum::general.cancel') }}</button>
                    <button type="submit" class="btn btn-danger fggg">{{ trans('forum::general.proceed') }}</button>
                </div>
            </form>
        </div>
    </div>
...
<script src="{{ asset('assets/plugins/jquery/jquery.min.js') }}"></script>
    ...
    $('.report_user_button').click(function(e){
        
        author_id = $(this).attr('author_id');

        $('form.report_user_form input.hidden_report_user_id').val(author_id);
