...
@section('content')
    <div class="d-flex flex-row justify-content-betweenz mb-2 category-header">
        <img loading="lazy" class="lazy relative lazyloaded fleft" src="{{asset('admin/category_images/' . $category->category_image)}}" alt="profile image" width="72" height="72" style="border-radius: 9px;">
        <h2 style="color: {{ $category->color }};">
            {{ $category->title }} &nbsp;<br>
            @if ($category->description)
                <small><h6>{{ $category->description }}</h6></small>
            @endif
        </h2>
    </div>
    ...
    @if (! $threads->isEmpty())
        @can ('markThreadsAsRead')
            <div class="text-center mt-3 dnone">
